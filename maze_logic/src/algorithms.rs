...
pub fn find_furthest<S: Shape>(maze: &Maze<S>, start: usize) -> usize {
    ...
    while let Some(current) = queue.pop_back() {
        ...
        for neighbour in maze.get_accessible_neighbours(current) {
...
pub fn find_path<S: Shape>(maze: &Maze<S>, from: usize, to: usize) -> Vec<usize> {
    let mut visited = vec![false; maze.size()];
